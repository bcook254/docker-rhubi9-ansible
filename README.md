# Red Hat Univeral Base Image 9 Ansible Test Image
...
## How to Use
  ...
  1. [Install Docker](https://docs.docker.com/engine/installation/).
  2. Pull this image from Docker Hub: `docker pull bcook254/docker-rhubi9-ansible:latest` (or use the image you built earlier, e.g. `rhubi9-ansible:latest`).
  3. Run a container from the image: `docker run --detach --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:rw --cgroupns=host bcook254/docker-rhubi9-ansible:latest` (to test my Ansible roles, I add in a volume mounted from the current working directory with ``--volume=`pwd`:/etc/ansible/roles/role_under_test:ro``).
